...
const assetPrices = [
  // {asset: 'EMB', denomination: 'USDT', price : 0},
  ...
  //
  // {asset: 'EMB', denomination: 'DAI', price : 0},
  ...
  //
  // {asset: 'EMB', denomination: 'BTC', price : 0},
...
export default async function getPrices() {
  const jsonData: { usdt: number, dai: number, emb: number, nrg: number, btc: number, wav: number, eth: number, neo: number, lsk:number }
    = {usdt: 1, dai: 1, emb: 0.5, nrg: 0.5, btc:0,wav:0,lsk:0,eth:0,neo:0}
  ...
  console.log({assetPrices,jsonData});
